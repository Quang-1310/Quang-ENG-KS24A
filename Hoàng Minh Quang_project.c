...
void returnBook(member b[], int m, book *a, int n){
    ...
    for(int i = 0; i < m; i++){
        if(idMember == b[i].memberId){
            flag = 1;
            if(b[i].nBB == 0){ 
                printf("Member With ID %d No Books to Return.\n", b[i].memberId);
                break;
            } 
			else{
                printf("Enter the Book ID You Want to Return: ");
                scanf("%d", &idBook);              
                if(strcmp(b[i].borrowedBook, a[idBook - 1].title) != 0){
                    printf("The Book ID You Entered Does Not Match the Book You Are Borrowing.\n");
                    break;
                }
                for(int j = 0; j < n; j++){
                    if(idBook == a[j].bookId){
                        printf("Enter Number Of Books Want To Return: ");
                        scanf("%d", &numberBook);
                        if(numberBook > 0 && numberBook <= b[i].nBB){
						...
						else if(numberBook > b[i].nBB){
                            printf("You Cannot Return More Books Than You Borrowed!\n");
                            break;
                        } 
						else{
                            printf("Invalid Number of Books to Return!\n");
                            break;
                        }
                    }
                }
